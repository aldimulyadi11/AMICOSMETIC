...
<body>
  <center>
    <p><strong>Laporan Pembelian</strong><br>
      <br><br>
  </center>
  ...
  <table rules="all" width="100%" cellpadding="2" >
    ...
    <tbody>
        @foreach($pembelian as $data)
            <tr>
              ...
              <td>Rp. {{number_format($data->harga,0, ".", ".")}}</strong></td>
              <td>{{$data->qty}}</td>
              <td>Rp. {{number_format($data->total,0, ".", ".")}}</strong></td>
              <td>{{$data->tanggal}}</td>
            </tr>
      </tr>
      @endforeach
    ...
    <tfoot>
      <tr>
    <td style="text-align: center;" colspan="5"><strong>Total Pembelian</strong></td>
